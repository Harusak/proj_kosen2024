function setFig(fig, x, y, fs, fn, ppt)
    ...
    fig.InnerPosition = [3, 3, x, y];
    fig.PaperType     = '<custom>';
    fig.PaperSize = [x,y] ;
    fig.PaperPosition = [0, 0, x, y] ;
    ...
    for iAxes = 1 : nAxes
        if isa(fig.Children(iAxes), 'matlab.graphics.axis.Axes')
            ...
            if ~ppt
                ...
                set(fig.Children(iAxes), 'XColor', 'k', 'YColor', 'k', 'ZColor', 'k');
            end
            ...
            if ~isempty(fig.Children(iAxes).Legend)
                fig.Children(iAxes).Legend.FontSize = fs;
            end
